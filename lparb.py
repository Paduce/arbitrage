...
def LPGraph(nodes):
    ...
    paths = nx.all_simple_paths(G, source='0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c',
                                target='0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee',cutoff=6)
    path = []
    for k in list(paths):
...
path = LPGraph([wbnb,busd,cake,bunny,usdc,btcb,ethb,safemoon,ust,dai,dot,ada,xrp,link])
    ...
    for i in range(0,path.shape[0]):
         try:
            for j in range(0,len(path.iloc[i,:].dropna())):
                rip = path.iloc[i, j]
                LP_address = contract.functions.getPair(str(path.iloc[i,j][0]), str(path.iloc[i,j][1])).call()
                if LP_address == "0x0000000000000000000000000000000000000000":
                    raise ValueError
                w3.eth.contract(address=LP_address, abi=pair_abi)

         except:
             for t in range(path.shape[1]):
                path = path[np.logical_and(path.iloc[:,t].astype(str) != str(rip),path.iloc[:,t].astype(str) != str(reversed(rip)))]
             continue
    print("data cleaned")
    return path

start = time.time()
data_cleaned = DataCleaning(data)
print(time.time() - start)
data_cleaned.to_csv("out_cleaned.csv",index=False)


data_cleaned = pd.DataFrame(pd.read_csv("out_cleaned.csv",sep='"', delimiter=None))
def LP_addreses(path):
    for i in range(0,path.shape[0]):
         amounts = [10**18]

         try:
            for j in range(0,len(path.iloc[i,:].dropna())-1):
                LP_contract = w3.eth.contract(address=contract.functions.getPair(str(path.iloc[i,j][2:44]), str(path.iloc[i,j][48:90])).call(), abi=pair_abi)
